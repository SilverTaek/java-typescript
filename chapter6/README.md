...
# 1. 타입스크립트로만 작성한 모듈을 외부에 공개 후 설치되는 과정에서 ts compiler를 사용해서 트랜스파일링이 되어야 한다고 했을 때
     외부 사용자가 운영 환경에서 해당 모듈 설치 시 `Devdependencies` 에 위치한 package 는 설치가 안될텐데, 어떻게 대처하면 좋을까? 
