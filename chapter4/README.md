...
## 김지섭
함수를 작성함에 있어 매개변수의 타입은 범위가 넓어질 수 있으나 함수가 반환하는 값의 매개변수의 타입에 비해서는 최대한 자세하고 명료하게 적어주는 것이 더 좋은 코드임을 알게되었다. 반환타입을 자세하고 명료하게 쓰기 위해 Omit, Partial 등의 유틸리티 타입을 이용할 수 있다. 또한 string 타입보다는 더 구체적인 타입을 사용하는 것이 더 좋은 코드임을 알게되었습니다. 데이터를 보지 않고 API 명세를 보고 타입을 생성하면 좋다는 것이 인상적이었습니다.
